$(document).ready(function() {
    ...
    //     if (!$(e.target).closest('#profile-modal').length && !$(e.target).closest('#profile-btn').length) {
    ...
    if (currentPath === '/map') {
        ...
        $('main').css('max-width', 'none').css('height', 'calc(100vh - 67px');
    ...
    function close() {
        ...
        $('#agree-service, #agree-privacy, #all-agree').prop('checked', false);
        $('#btn-next-step-1').prop('disabled', true);
    ...
    $('#btn-login').on('click', function(e) {
        ...
        if (!email || !password) {
            $('#error-message').html('아이디와 비밀번호를 입력해주세요.'); return;
        } else {
            $.ajax({
    ...
    function navigateToStep(step) {
        ...
        // 이메일, 비밀번호 입력창에서만 sns 간편로그인 부분 보이기
        if (currentStep === 2) {
            $('.sns-login-wrapper').removeClass('hidden');
        } else {
            $('.sns-login-wrapper').addClass('hidden');
        }
    }
...
c 
    ...
    $('input[name="memberName"], input[name="member_phone_middle"], input[name="member_phone_back"], input[name="birthday"]').on('input', function () {
        ...
        $('#btn-register').prop('disabled', !(validatePhone && validateBirthday && validateName));
    });
    ...
    const Popup = (() => {
        ...
        <div id="slide-popup" class="popup">
            <div class="popup-content com-bg com-color com-border com-shadow com-round">
                <span class="popup-message"></span>
                <button id="popup-close-btn" class="com-btn-primary com-border com-round">닫기</button>
            </div>
        </div>
        `;
            ...
            if (popupElement) {
                ...
                setTimeout(() => {
                    ...
                    if (typeof customCloseCallback === 'function') {
                        customCloseCallback();
                        customCloseCallback = null; // 콜백 초기화
                    }
                }, 500); 
            }
        }
